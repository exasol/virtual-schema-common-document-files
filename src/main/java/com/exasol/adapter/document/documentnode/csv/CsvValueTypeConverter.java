...
class CsvValueTypeConverter {
    ...
    static CsvValueTypeConverter create(final List<ColumnMapping> csvColumns) {
        final List<ColumnMapping> filterColums = csvColumns.stream()
        ...
        int index = 0;
        for (final ColumnMapping columnMapping : filterColums) {
    ...
    private static String getColumnName(final ColumnMapping columnMapping) {
    ...
    private static IllegalArgumentException unsupportedMappingException(final ColumnMapping columnMapping) {
    ...
    private static String getColumnNameFromPath(final PropertyToColumnMapping propertyMapping) {
        if (propertyMapping.getPathToSourceProperty().getSegments().size() != 1) {
            throw new IllegalArgumentException(ExaError.messageBuilder("E-VSDF-63")
                    .message("Path to source {{path}} has {{segment count}} segments.",
                            propertyMapping.getPathToSourceProperty(), propertyMapping.getPathToSourceProperty().size())
        ...
        if (!(segment instanceof ObjectLookupPathSegment)) {
            throw new IllegalArgumentException(ExaError.messageBuilder("E-VSDF-64")
                    .message("Segment {{segment}} of path {{path}} has an unsupported type {{segment type}}.", segment,
                            propertyMapping.getPathToSourceProperty(), segment.getClass().getName())
    ...
    private static ValueConverter createConverter(final ColumnMapping columnMapping) {
        if (columnMapping instanceof PropertyToVarcharColumnMapping) {
            ...
            return CsvValueTypeConverter::booleanConverter;
        }
        return StringHolderNode::new;
    }
