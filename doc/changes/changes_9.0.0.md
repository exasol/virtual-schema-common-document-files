# Virtual Schema for Document Data in Files 9.0.0, released 2026-05-08

Code name: Anonymous feature tracking

## Summary

This release adds anonymous feature tracking using the [telemetry-java](https://github.com/exasol/telemetry-java) library. When you integrate this new version into another product, please check the [required user documentation](https://github.com/exasol/telemetry-java/blob/main/doc/integration-guide.md#required-documentation).

## Features

* #201: Added anonymous feature tracking

## Dependency Updates

### Compile Dependency Updates

* Updated `com.exasol:error-reporting-java:1.0.1` to `1.0.2`
* Updated `com.exasol:parquet-io-java:2.0.13` to `2.0.15`
* Updated `com.exasol:virtual-schema-common-document:11.0.7` to `12.0.2`
* Updated `io.deephaven:deephaven-csv:0.15.0` to `0.19.0`

### Test Dependency Updates

* Updated `com.exasol:hamcrest-resultset-matcher:1.7.1` to `1.7.2`
* Updated `com.exasol:performance-test-recorder-java:0.1.4` to `0.1.5`
* Updated `com.exasol:udf-debugging-java:0.6.17` to `0.6.18`
* Updated `com.exasol:virtual-schema-common-document:11.0.7` to `12.0.2`
* Updated `nl.jqno.equalsverifier:equalsverifier:3.19` to `3.19.4`
* Updated `org.junit.jupiter:junit-jupiter-params:5.13.0` to `5.14.4`
* Updated `org.mockito:mockito-junit-jupiter:5.18.0` to `5.23.0`
* Added `org.slf4j:slf4j-jdk14:1.7.36`

### Plugin Dependency Updates

* Updated `com.exasol:error-code-crawler-maven-plugin:2.0.6` to `2.0.7`
* Updated `com.exasol:project-keeper-maven-plugin:5.4.6` to `5.6.2`
* Updated `io.github.git-commit-id:git-commit-id-maven-plugin:9.0.2` to `10.0.0`
* Added `org.apache.maven.plugins:maven-failsafe-plugin:3.5.5`
* Updated `org.apache.maven.plugins:maven-jar-plugin:3.4.2` to `3.5.0`
* Updated `org.apache.maven.plugins:maven-resources-plugin:3.4.0` to `3.5.0`
* Updated `org.apache.maven.plugins:maven-surefire-plugin:3.5.4` to `3.5.5`
